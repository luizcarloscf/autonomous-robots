...
: Node("turtlebot_controller")
{
  ...
  this->subscriber_odom_ = this->create_subscription<nav_msgs::msg::Odometry>(
    "odom", 10,
    std::bind(
      &TurtlebotController::callback_odometry, this, std::placeholders::_1));
  ...
  this->action_server_ = rclcpp_action::create_server<Task>(
    ...
    "task",
    std::bind(
      &TurtlebotController::callback_goal, this,
      std::placeholders::_1, std::placeholders::_2),
    std::bind(
      &TurtlebotController::callback_cancel, this,
      std::placeholders::_1),
    std::bind(
      &TurtlebotController::callback_accepted, this,
      std::placeholders::_1));
  // add parameter handler
  this->parameters_ = this->add_on_set_parameters_callback(
    std::bind(
...
void TurtlebotController::execute()
{
  ...
  while (!this->queue_.empty()) {
    ...
    for (std::size_t i = 0; i < goals->x.size(); i++) {
      ...
      auto distance = this->euclidian_distance(
      ...
      while (distance > this->tolerance && rclcpp::ok()) {
        ...
        double alpha = this->steering_angle(
          current->pose.pose.position.x,
          current->pose.pose.position.y,
          goals->x[i],
          goals->y[i]);
        ...
        current = this->odometry_;
        distance = this->euclidian_distance(
          current->pose.pose.position.x,
          current->pose.pose.position.y,
          goals->x[i],
          goals->y[i]);
...
double TurtlebotController::steering_angle(
  double x1,
  double y1,
  double x2,
  double y2)
...
double TurtlebotController::euclidian_distance(
  double x1,
  double y1,
  double x2,
  double y2)
